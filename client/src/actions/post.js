...
export const createLike = (postId, TYPE) => (dispatch) => {
  axios.post(`/api/posts/${postId}/likes`, null, getConfig()).then((res) =>
    dispatch({
      type: TYPE,
      payload: res.data,
    })
...
export const removeLike = (postId, likeId, TYPE) => (dispatch) => {
  axios
    .delete(`/api/posts/${postId}/likes/${likeId}`, getConfig())
    .then((res) =>
      dispatch({
        type: TYPE,
        payload: res.data,
...
export const createComment = (postId, comment) => (dispatch) => {
  axios
    .post(`/api/posts/${postId}/comments`, comment, getConfig())
    .then((res) => {
      if (comment !== "") {
        dispatch({
          type: UPDATE_POST,
...
export const removeComment = (postId, commentId) => (dispatch) => {
  axios
    .delete(`/api/posts/${postId}/comments/${commentId}`, getConfig())
    .then(
      (res) =>
        dispatch({
          type: UPDATE_POST,
